<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $ID_Centro_Destinado = isset($_POST['ID_Centro']) ? htmlspecialchars($_POST['ID_Centro']) : '';  
...
<body>
    <?php
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['Tipo']) && $_POST['Tipo'] == "1") {
                ...
                $ID_Centro_Destinado = isset($_POST['ID_Centro_Destinado']) ? htmlspecialchars($_POST['ID_Centro_Destinado']) : '';  
                $Observaciones= isset($_POST['Observaciones']) ? htmlspecialchars($_POST['Observaciones']) : '';  
                $Firma = isset($_POST['firma']) ? $_POST['firma'] : '';
                
                $ProductosController->FirmarTraslado($No_Documento,$ID_Recibe, $ID_Usuario, $ID_Centro, $ID_Centro_Destinado, $Firma, $Nombre_Ingresa, $Observaciones);
            }
        }        
    ?>   
    <h2>Firma Traslado</h2>
    ...
    <form method="post">
        ...
        <input type="hidden" name="ID_Centro_Destinado" value="<?=$ID_Centro_Destinado?>">
