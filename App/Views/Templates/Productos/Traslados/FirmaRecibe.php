<?php
    ...
    $ID_Traslado = $_GET['Traslado'];
    $DataUsuario = $DotacionController->BuscarPersona($No_Documento);
    $estadoFirma = $ProductosController->verificarEstadoFirmaT($ID_Traslado); 
    ...
    if ($estadoFirma['Firma_Estado_Recibe'] === 1) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    title: 'Atención',
                    text: 'Ya has firmado este Traslado.',
                    icon: 'warning',
...
<body>
    <?php
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['Tipo']) && $_POST['Tipo'] == "1") {
                ...
                $ID_Traslado = isset($_POST['ID_Traslado']) ? htmlspecialchars($_POST['ID_Traslado']) : '';
                ...
                $ProductosController->FirmarTrasladoRecibe($ID_Traslado, $No_Formulario,$ID_Recibe, $Nombre_Ingresa, $Firma);
            }
        }        
    ?>   
    <h2>Firma Recibe</h2>
    ...
    <form method="post">
        ...
        <input type="hidden" name="ID_Traslado" value="<?=$_GET['Traslado']?>">
