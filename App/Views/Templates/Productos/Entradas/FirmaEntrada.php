    ...
    <?php
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['Tipo']) && $_POST['Tipo'] == "1") {
                ...
                $Nombre_Ingresa = isset($_POST['Nombre_Ingresa']) ? htmlspecialchars($_POST['Nombre_Ingresa']) : '';
                $ID_Centro = isset($_POST['ID_Centro']) ? htmlspecialchars($_POST['ID_Centro']) : '';
                $Observaciones = isset($_POST['Observaciones']) ? htmlspecialchars($_POST['Observaciones']) : '';
                $Firma = isset($_POST['firma']) ? $_POST['firma'] : '';
                $ProductosController->FirmarEntrada($ID_Usuario, $ID_Supervisor, $ID_Centro, $Firma, $Nombre_Ingresa, $CorreoSupervisor, $Nombre_Supervisor, $Observaciones);
            }
        }        
    ?>   
    <h2>Firma Entrada</h2>
