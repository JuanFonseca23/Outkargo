<?php
    ...
    $entradaPendiente = $ProductosController->verificarEstadoEntrada($_SESSION['ID']); 
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        if ($_POST['Tipo'] == "1") {
                            ...
                            $ValorUnitario = !empty($_POST['Valor']) ? $_POST['Valor'] : 0;
                            $Factura = !empty($_POST['Factura']) ? $_POST['Factura'] : NULL;
                            $ProductosController->InsertarEntradaTemp($Codigo, $ValorCantidad, $Medida, $ValorUnitario, $Factura, $ID_Usuario);
                        }  
                    }
                ?>
                <form method="post">
                    ...
                    <th width="500px">Valor Unitario</th>
                    <td width="50px"><input name="Valor"></td>
                    <th width="500px">Número de Factura</th>
                    <td width="50px"><input name="Factura"></td>
                    <input type="hidden" name="Usuario" value="<?= $_SESSION['ID'] ?>">
                    <input type="hidden" name="Tipo" value="1">
                    <td><input type="submit" id="button1" style="background-color:rgb(0, 162, 49); color: white; border: none; padding: px; cursor: pointer; align-items: center; justify-content: center; border-radius: 10%; box-sizing: border-box; font-size: 14px; font-weight: bold;" value="Agregar"></td>
                    <th width="700px"></th>
                </form> 
            </tr>
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        ...
                        if ($_POST['Tipo'] == "2") {
                            ...
                            $ID_Usuario = $_POST['Usuario'];
                            $ID = $_POST['ID'];
                            $ProductosController->RestarEntradaTemp($Codigo, $ID_Usuario, $ID);
                        }    
                        if ($_POST['Tipo'] == "3") {
                            $Codigo = $_POST['Codigo'];
                            $ID_Usuario = $_POST['Usuario'];
                            $ID = $_POST['ID'];
                            $ProductosController->EliminarEntradaTemp($Codigo, $ID_Usuario, $ID);
                        }                       
                    }
                ?>
