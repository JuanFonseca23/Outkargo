<?php
    ...
    $ID_Salida = $_GET['ID']; 
    $estadoFirma = $ProductosController->verificarEstadoFirmaSalida1($ID_Salida); 

    if ($estadoFirma['Firma_Estado_Salida'] === 1) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    title: 'Atención',
                    text: 'Ya has autorizado esta salida.',
                    icon: 'warning',
...
<body>
    <?php
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
                ...
                $ID = $_GET['ID'];
                $No_Formulario = $_GET['No_Formulario'];
                $ProductosController->FirmarSalida1($ID, $Usuario, $Nombre_Ingresa, $No_Formulario, $Firma_Supervisor);                                                   
            }
    ?>
    <style>
        #signature-pad {
