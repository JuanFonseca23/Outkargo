<?php
    ...
    $ListaMontacargas = $ProductosController->ListaMontacargas($_GET['Centro']);
    $DataSupervisores = $UsuariosController->obtenerSupervisor();
    $Filas = $ProductosController->DetallesSalida($_SESSION['ID']);
...
<body>
    ...
    <?php       
        if ($dataUltimaSalida = $ProductosController->TraerUltimoRegistroSalida($_SESSION['ID'])) {
            ...
            $ProductosController->EliminarUltimoRegistroSalida($_SESSION['ID']);
        }
    ?>
    <table border="1">
        <thead>
            <tr>
                ...
                <th rowspan="3" class="Titulo" style="font-size: 20px;">FORMATO SALIDA DE PRODUCTOS</th>
                <th>CODIGO:</th>
                <td>F-250</td>
            </tr>
            <tr>
    ...
    <table border="1">
        <thead>
            <tr>
                ...
                <th>Centro Salida:</th>
                <td>
                    <?= htmlspecialchars($_SESSION['Centro']) ?>
                </td>
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        ...
                        if ($_POST['Tipo'] == "2") {
                            ...
                            $ID_Usuario = $_POST['Usuario'];
                            $ID = $_POST['ID'];
                            $ProductosController->RestarSalidaTemp($Codigo, $ID_Usuario, $ID, $No_Documento, $CentroTrabajo);
                        }    
                        if ($_POST['Tipo'] == "3") {
                            $Codigo = $_POST['Codigo'];
                            $ID_Usuario = $_POST['Usuario'];
                            $ID = $_POST['ID'];
                            $ProductosController->EliminarSalidaTemp($Codigo, $ID_Usuario, $ID, $No_Documento, $CentroTrabajo);
                        }                       
                    }
                ?>
    ...
    <form id="GuardarEntrada" action="FirmaSalida?Documento=<?= $No_Documento ?>" method="post">
        <table border="1">
            <thead>
                <tr>
                    <th>Salida</th>
        ...
        <input type="hidden" name="ID_CentroDestinado" value="<?=$_GET['Centro']?>">
