<?php
    ...
    $DataMantenimiento = $MantenimientoController->ObtenerMantenimientoC($_GET['ID']); 
    $Estado_Firma_Operario = $DataMantenimiento['Estado_Firma_Operario'];

    if ($DataMantenimiento['Estado_Firma_Operario'] === 1) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    title: 'Atención',
                    text: 'Ya has firmado este mantenimiento.',
                    icon: 'warning',
    ...
    <?php
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['Tipo']) && $_POST['Tipo'] == "1") {
                ...
                $MantenimientoController->FirmarMantenimientoOperarioCorrectivo($ID_Mantenimiento, $Firma);
            }
        }        
    ?>   
    <h2>Firma Operario</h2>
